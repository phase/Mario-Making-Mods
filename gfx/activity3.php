<?php
 ...
 $user['regdate']	= fetchResult("SELECT MIN(`regdate`) FROM users");

 $max				= ceil((fetchResult("SELECT COUNT(*) FROM `posts`") + 1) / 5000) * 5000;
...
// $max				= 5500;
 ...
 function drawdata($p, $color) {
  ...
  function getdata($u) {
	 ...
	 $nn	= query("SELECT FROM_UNIXTIME(date,'%Y%m%d') ymd, floor(date/86400) d, count(*) c FROM posts  GROUP BY ymd ORDER BY ymd");
